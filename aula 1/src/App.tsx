...
import BankMovement from "./classes/bankMovement";
...
function App() {
  ...
  const [bankEdited, setBankEdited] = useState<BankAccount>(
    new BankAccount({ name: "null" }, "economica")
  );
  ...
  const banksTypes: ["total", "simples", "economica"] = [
    "total",
    "simples",
    "economica",
  ];
  ...
  function addBank(clientIndex: number, bankTypeIndex: number) {
    if (addBankClientInput.current && addBankTypeInput.current) {
      ...
      if (bankAccounts.filter((data) => data.client === client).length === 0) {
        setBankAccounts((prev) => [
          ...prev,
          new BankAccount(client, banksTypes[bankTypeIndex]),
        ]);
      }
    }
  }
  ...
  function changeType(typeIndex: number, bank: BankAccount) {
    let newBank = bank;

    newBank.bankType = banksTypes[typeIndex];
  ...
  function addBankMovement(
    ...
    if (
      addBankMovementAmountInput.current &&
      addBankMovementDescriptionInput.current
    ) {
      let newBank = bank;
    ...
    <main className="h-screen p-12 bg-black text-white flex flex-col justify-between items-center">
      ...
      <div className="flex justify-center gap-12">
        <div className="w-[40vw] h-[40vh] overflow-y-auto flex flex-col border border-white rounded">
          <p className="p-4 bg-white text-black border-b border-white">
            Clientes:
          </p>
          {clients.map((data, index) => (
            <div
              key={`cliente - ${index}`}
              className="p-4 odd:bg-neutral-800 odd:border-y border-white py-4"
            >
            ...
            >
              ...
              <div className="flex justify-evenly gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (changeBankTypeInput.current)
                        changeType(
                          changeBankTypeInput.current.selectedIndex,
                          data
                        );
                    }}
                    className="border border-white rounded hover:bg-neutral-800 transition p-2"
      ...
      <div className="flex gap-4 border border-white rounded p-4">
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              if (addClientNameInput.current)
                addClient(addClientNameInput.current.value);
            }}
            className="border border-white rounded hover:bg-neutral-800 transition p-2"
          >
            Adicionar cliente
        ...
        <div className="flex gap-2">
          ...
          >
            {banksTypes.map((data) => (
              <option key={`Fazer banco - ${data}`} className="bg-black">
                {data}
              </option>
            ))}
          </select>
          <button
            onClick={() => {
              if (addBankClientInput.current && addBankTypeInput.current)
                addBank(
                  addBankClientInput.current.selectedIndex,
                  addBankTypeInput.current.selectedIndex
                );
            }}
            className="border border-white rounded hover:bg-neutral-800 transition p-2"
          >
